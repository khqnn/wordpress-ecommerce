...
class magical_plugin_activation_Plugin_Recommendations {
    ...
    public function add_admin_menu() {
        add_theme_page(
            __('Recommended Plugins', 'wowmart'),
            __('Recommended Plugins', 'wowmart'),
            'manage_options',
    ...
    public function get_recommended_plugins() {
        ...
        $default_plugins = array(
            'easy-share-solution' => array(
                'name' => __('Easy Share Solution', 'wowmart'),
                'slug' => 'easy-share-solution',
                'file' => 'easy-share-solution/easy-share-solution.php',
                'description' => __('Easily add social sharing buttons to your content.', 'wowmart'),
                'category' => 'Utility',
                'required' => false,
                'featured' => true,
                'is_local' => false
            ...
            'wp-edit-password-protected' => array(
                'name' => __('WP Edit Password Protected', 'wowmart'),
                'slug' => 'wp-edit-password-protected',
                'file' => 'wp-edit-password-protected/wp-edit-password-protected.php',
                'description' => __('Easily manage password protection for your WordPress content.', 'wowmart'),
                'category' => 'Security',
                'required' => false,
                'featured' => true,
                'is_local' => false
    ...
    public function get_plugin_status($plugin) {
        ...
        if ($this->is_plugin_active_check($plugin_file)) {
    ...
    /**
     * Check if a plugin is active (reliable alternative to is_plugin_active)
     * 
     * @param string $plugin_file Plugin file path relative to plugins directory
     * @return bool True if plugin is active, false otherwise
     */
    private function is_plugin_active_check($plugin_file) {
    ...
    public function render_admin_page() {
        ...
        <div class="wrap magical-plugin-activation-plugins">
            <h1><?php printf(esc_html__('Recommended Plugins for %s', 'wowmart'), $this->theme_name); ?></h1>
            <p class="description">
                <?php printf(esc_html__('These plugins are recommended to enhance your %s theme experience. They work perfectly with the theme\'s features and animations.', 'wowmart'), $this->theme_name); ?>
            </p>
            ...
            <div class="notice notice-warning">
                <p>
                    <strong><?php esc_html_e('Missing Required Plugins:', 'wowmart'); ?></strong>
                    <?php printf(esc_html__('%d required plugins are missing or inactive.', 'wowmart'), count($missing_required)); ?>
                    <button id="install-required-plugins" class="button button-primary" style="margin-left: 10px;">
                        <?php esc_html_e('Install Required Plugins', 'wowmart'); ?>
                    </button>
                </p>
            </div>
            <?php endif; ?>
            ...
            <div class="plugin-filter-tabs">
                <?php if ($plugin_counts['all'] > 0) : ?>
                    <button class="filter-tab <?php echo esc_attr(($first_active_tab === 'all') ? 'active' : ''); ?>" data-filter="all"><?php printf(esc_html__('All Plugins (%d)', 'wowmart'), $plugin_counts['all']); ?></button>
                <?php endif; ?>
                
                <?php if ($plugin_counts['featured'] > 0) : ?>
                    <button class="filter-tab <?php echo esc_attr(($first_active_tab === 'featured') ? 'active' : ''); ?>" data-filter="featured"><?php printf(esc_html__('Featured (%d)', 'wowmart'), $plugin_counts['featured']); ?></button>
                <?php endif; ?>
                
                <?php if ($plugin_counts['required'] > 0) : ?>
                    <button class="filter-tab <?php echo esc_attr(($first_active_tab === 'required') ? 'active' : ''); ?>" data-filter="required"><?php printf(esc_html__('Required (%d)', 'wowmart'), $plugin_counts['required']); ?></button>
                <?php endif; ?>
                
                <?php if ($plugin_counts['active'] > 0) : ?>
                    <button class="filter-tab <?php echo esc_attr(($first_active_tab === 'active') ? 'active' : ''); ?>" data-filter="active"><?php printf(esc_html__('Active (%d)', 'wowmart'), $plugin_counts['active']); ?></button>
                <?php endif; ?>
                
                <?php if ($plugin_counts['inactive'] > 0) : ?>
                    <button class="filter-tab <?php echo esc_attr(($first_active_tab === 'inactive') ? 'active' : ''); ?>" data-filter="inactive"><?php printf(esc_html__('Inactive (%d)', 'wowmart'), $plugin_counts['inactive']); ?></button>
                <?php endif; ?>
                
                <?php if ($plugin_counts['not-installed'] > 0) : ?>
                    <button class="filter-tab <?php echo esc_attr(($first_active_tab === 'not-installed') ? 'active' : ''); ?>" data-filter="not-installed"><?php printf(esc_html__('Not Installed (%d)', 'wowmart'), $plugin_counts['not-installed']); ?></button>
                <?php endif; ?>
            </div>
                ...
                ?>
                    <div class="plugin-card <?php echo esc_attr($featured_class); ?>" data-status="<?php echo esc_attr($status); ?>" data-featured="<?php echo esc_attr($plugin['featured'] ? '1' : '0'); ?>" data-required="<?php echo esc_attr($plugin['required'] ? '1' : '0'); ?>" data-local="<?php echo esc_attr($is_local ? '1' : '0'); ?>" data-slug="<?php echo esc_attr($slug); ?>">
                        <?php if ($plugin['featured']) : ?>
                            <div class="featured-badge"><?php esc_html_e('Featured', 'wowmart'); ?></div>
                        <?php endif; ?>
                        
                        <?php if ($plugin['required']) : ?>
                            <div class="required-badge"><?php esc_html_e('Required', 'wowmart'); ?></div>
                        <?php endif; ?>
                        
                        <?php if ($is_local) : ?>
                            <div class="local-plugin-badge"><?php esc_html_e('Local', 'wowmart'); ?></div>
                        <?php endif; ?>
    ...
    private function render_plugin_button($slug, $plugin, $status) {
        ...
        switch ($status) {
            case 'active':
                echo '<span class="button disabled">' . esc_html__('Active', 'wowmart') . '</span>';
                break;
            case 'needs-update':
                echo '<button class="button button-secondary update-plugin" data-slug="' . esc_attr($slug) . '" data-file="' . esc_attr($plugin['file']) . '" data-is-local="' . ($is_local ? '1' : '0') . '">' . esc_html__('Update', 'wowmart') . '</button>';
                break;
            case 'inactive':
                echo '<button class="button button-primary activate-plugin" data-slug="' . esc_attr($slug) . '" data-file="' . esc_attr($plugin['file']) . '">' . esc_html__('Activate', 'wowmart') . '</button>';
                break;
            case 'inactive-needs-update':
                echo '<button class="button button-secondary update-plugin" data-slug="' . esc_attr($slug) . '" data-file="' . esc_attr($plugin['file']) . '" data-is-local="' . ($is_local ? '1' : '0') . '">' . esc_html__('Update', 'wowmart') . '</button>';
                break;
            case 'not-installed':
                if ($is_local) {
                    echo '<button class="button button-primary install-plugin" data-slug="' . esc_attr($slug) . '" data-is-local="1">' . esc_html__('Install', 'wowmart') . '</button>';
                } else {
                    echo '<button class="button button-primary install-plugin" data-slug="' . esc_attr($plugin['slug']) . '" data-is-local="0">' . esc_html__('Install', 'wowmart') . '</button>';
                }
                break;
        }
        ...
        if ($current_version && $required_version) {
            ...
            printf(esc_html__('Current: %s | Required: %s', 'wowmart'), $current_version, $required_version);
            echo '</small>';
        } elseif ($current_version) {
            echo '<small class="version-info">';
            printf(esc_html__('Version: %s', 'wowmart'), $current_version);
            echo '</small>';
        } elseif ($required_version) {
            echo '<small class="version-info">';
            printf(esc_html__('Required Version: %s', 'wowmart'), $required_version);
            echo '</small>';
        }
    }
    ...
    private function get_status_text($status) {
        switch ($status) {
            case 'active':
                return __('Active', 'wowmart');
            case 'needs-update':
                return __('Update Available', 'wowmart');
            case 'inactive':
                return __('Installed but not active', 'wowmart');
            case 'inactive-needs-update':
                return __('Update Available (Inactive)', 'wowmart');
            case 'not-installed':
                return __('Not installed', 'wowmart');
            default:
                return __('Unknown', 'wowmart');
        }
    }
    ...
    public function show_update_notice() {
        ...
        <div class="notice notice-warning is-dismissible magical-plugin-activation-pro-update-notice">
            <div class="plugin-update-content">
                <div style="flex: 1;">
                    <h3><?php esc_html_e('Plugin Updates Available', 'wowmart'); ?></h3>
                    <p><?php printf(esc_html__('%d plugins have updates available. Keep your plugins up to date for better security and performance.', 'wowmart'), count($plugins_need_update)); ?></p>
                    <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap; margin-top: 15px;">
                        <button id="update-all-plugins" class="button button-primary"><?php esc_html_e('Update All Plugins', 'wowmart'); ?></button>
                        <a href="<?php echo esc_url(admin_url('themes.php?page=magical-plugin-activation-plugins')); ?>" class="button button-secondary"><?php esc_html_e('Manage Plugins', 'wowmart'); ?></a>
                    </div>
                </div>
    ...
    public function ajax_dismiss_install_notice() {
        ...
        update_user_meta(get_current_user_id(), 'magical_plugin_activation_hide_install_notice_time1', time());
        wp_send_json_success();
    }
    ...
    public function ajax_dismiss_plugin_notice() {
        ...
        if ($notice_type === 'recommended') {
            ...
            update_user_meta(get_current_user_id(), 'magical_plugin_activation_info_recomend_plugins1', time());
        }
    ...
    public function ajax_update_all_plugins() {
        ...
        if (!current_user_can('update_plugins')) {
            wp_die(__('You do not have permission to update plugins.', 'wowmart'));
        }
        ...
        if ($success_count === $total_count) {
            wp_send_json_success(array(
                'message' => sprintf(__('%d plugins updated successfully.', 'wowmart'), $success_count),
                'results' => $results
            ));
        } else {
            wp_send_json_error(array(
                'message' => sprintf(__('%d of %d plugins updated successfully.', 'wowmart'), $success_count, $total_count),
                'results' => $results
            ));
        }
    }
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        wp_localize_script('magical-plugin-activation-plugins-admin', 'MagicalPluginActivation', array(
            ...
            'strings' => array(
                'installing' => __('Installing...', 'wowmart'),
                'activating' => __('Activating...', 'wowmart'),
                'updating' => __('Updating...', 'wowmart'),
                'installed' => __('Installed', 'wowmart'),
                'activated' => __('Activated', 'wowmart'),
                'updated' => __('Updated', 'wowmart'),
                'active' => __('Active', 'wowmart'),
                'inactive' => __('Inactive', 'wowmart'),
                'notInstalled' => __('Not Installed', 'wowmart'),
                'needsUpdate' => __('Update Available', 'wowmart'),
                'inactiveNeedsUpdate' => __('Update Available (Inactive)', 'wowmart'),
                'install' => __('Install', 'wowmart'),
                'activate' => __('Activate', 'wowmart'),
                'update' => __('Update', 'wowmart'),
                'error' => __('Error occurred', 'wowmart'),
                'requiredWarning' => __('You must install and activate all required plugins before dismissing this notice. Click Dismiss again to hide this warning.', 'wowmart'),
            )
        ));
    ...
    public function ajax_install_plugin() {
        ...
        if (!current_user_can('install_plugins')) {
            wp_die(__('You do not have permission to install plugins.', 'wowmart'));
        }
        ...
        if ($is_local) {
            ...
            if (!isset($plugins[$slug])) {
                wp_send_json_error(__('Plugin not found in recommendations.', 'wowmart'));
            }
            ...
            if (!isset($plugin['source']) || !file_exists($plugin['source'])) {
                ...
                if (!isset($plugin['source']) || !file_exists($plugin['source'])) {
                    wp_send_json_error(__('Local plugin zip file not found. Please ensure the plugin zip file exists in the theme directory.', 'wowmart'));
                }
            }
            ...
            if (!is_writable(WP_PLUGIN_DIR)) {
                wp_send_json_error(__('Plugins directory is not writable. Please check directory permissions.', 'wowmart'));
            }
            
            $result = $this->install_local_plugin($plugin);
            
            if ($result) {
                wp_send_json_success(__('Local plugin installed successfully.', 'wowmart'));
            } else {
                wp_send_json_error(__('Local plugin installation failed. Please check error logs for details.', 'wowmart'));
            }
        } else {
            ...
            if (is_wp_error($api)) {
                wp_send_json_error(__('Plugin information could not be retrieved.', 'wowmart'));
            }
            ...
            if ($install) {
                wp_send_json_success(__('Plugin installed successfully.', 'wowmart'));
            } else {
                wp_send_json_error(__('Plugin installation failed.', 'wowmart'));
            }
        }
    }
    ...
    public function ajax_activate_plugin() {
        ...
        if (!current_user_can('activate_plugins')) {
            wp_die(__('You do not have permission to activate plugins.', 'wowmart'));
        }
        ...
        if (is_wp_error($result)) {
            ...
            wp_send_json_success(__('Plugin activated successfully.', 'wowmart'));
        }
    }
    ...
    public function ajax_update_plugin() {
        ...
        if (!isset($plugins[$slug])) {
            wp_send_json_error(__('Plugin not found.', 'wowmart'));
        }
        ...
        if ($is_local) {
            ...
            if ($result) {
                wp_send_json_success(__('Local plugin updated successfully.', 'wowmart'));
            } else {
                wp_send_json_error(__('Failed to update local plugin.', 'wowmart'));
            }
        } else {
            ...
            if ($result) {
                wp_send_json_success(__('Plugin updated successfully.', 'wowmart'));
            } else {
                wp_send_json_error(__('Failed to update plugin.', 'wowmart'));
            }
        }
    }
    ...
    public function ajax_install_required_plugins() {
        ...
        if ($success_count === $total_count) {
            wp_send_json_success(array(
                'message' => sprintf(__('%d required plugins installed and activated successfully.', 'wowmart'), $success_count),
                'results' => $results
            ));
        } else {
            wp_send_json_error(array(
                'message' => sprintf(__('%d of %d required plugins installed successfully.', 'wowmart'), $success_count, $total_count),
                'results' => $results
            ));
        }
    }
    ...
    public function ajax_install_recommended_plugins() {
        ...
        if ($success_count === $total_count) {
            wp_send_json_success(array(
                'message' => sprintf(__('%d recommended plugins installed and activated successfully.', 'wowmart'), $success_count),
                'results' => $results
            ));
        } else {
            wp_send_json_error(array(
                'message' => sprintf(__('%d of %d recommended plugins installed successfully.', 'wowmart'), $success_count, $total_count),
                'results' => $results
            ));
        }
    }
    ...
    public function ajax_check_recommended_plugins_status() {
        ...
        wp_send_json_success(array(
            ...
            'message' => $all_active ? __('All recommended plugins are active!', 'wowmart') : __('Some recommended plugins are not active.', 'wowmart')
        ));
    }
    ...
    public function show_recommendation_notice() {
        ...
        if (!empty($missing_required)) {
            ...
            $dismissed_time = get_user_meta(get_current_user_id(), 'magical_plugin_activation_hide_install_notice_time1', true);
            ...
            <div class="notice notice-warning magical-plugin-activation-pro-required-notice">
                <p>
                    ...
                    <?php printf(esc_html__('%d required plugins are missing. Please install them for optimal theme functionality.', 'wowmart'), count($missing_required)); ?>
                    <br><br>
    ...
    private function show_recommended_plugins_notice() {
        ...
        $dismissed_time = get_user_meta(get_current_user_id(), 'magical_plugin_activation_info_recomend_plugins1', true);
        ...
        <div class="notice notice-info is-dismissible magical-plugin-activation-pro-recommended-notice">
            <div class="plugin-recommended-content">
                <div style="flex: 1;">
                    <h3><?php printf(esc_html__('Recommended Plugins for %s', 'wowmart'), $this->theme_name); ?></h3>
                    <p><?php printf(esc_html__('We recommend %d additional plugins to enhance your %s theme experience.', 'wowmart'), count($inactive_recommended), $this->theme_name); ?></p>
                    <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap; margin-top: 15px;">
                        <button id="install-recommended-plugins" class="button button-primary"><?php esc_html_e('Install Recommended Plugins', 'wowmart'); ?></button>
                        <a href="<?php echo esc_url(admin_url('themes.php?page=magical-plugin-activation-plugins')); ?>" class="button button-secondary"><?php esc_html_e('View All Plugins', 'wowmart'); ?></a>
    ...
    private function update_local_plugin($plugin) {
        try {
            ...
            if ($this->is_plugin_active_check($plugin['file'])) {
