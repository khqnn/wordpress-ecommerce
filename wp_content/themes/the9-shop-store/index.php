...
*/
 do_action( 'the9_store_container_wrap_start',  esc_attr( $layout ) );
	...
	if ( have_posts() ) :
		...
		$column = ( the9_store_get_option('blog_layout') == 'full-container' ) ? 'col-md-4' : 'col-md-6';
        echo '<div id="masonry-grid" class="row grid">
                   <div class="grid-sizer '.esc_attr($column). ' blog-margin-top grid_layout"></div><div class="clearfix"></div>';
		/* Start the Loop */
		while ( have_posts() ) :
			the_post();
