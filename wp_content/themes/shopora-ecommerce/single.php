		...
		<main id="main" class="site-main">
			...
			while ( have_posts() ) : the_post();
				...
				if ( get_theme_mod( 'postpage_show_author' ) == '1' ) :
					...
					echo '<h3>';
					esc_html_e('About the author','shopora-ecommerce');
					echo '</h3>';
		...
		if ( get_theme_mod( 'postpage_show_hide_relatedposts' ) != '1' ) :
					...
					if ($categories) { $category_ids = array(); 
						...
						$my_query = new wp_query( $args ); if( $my_query->have_posts() ) {
							echo '<div class="related-posts"><div class="related-posts-headline"><h3>'.esc_html__('Explore more posts', 'shopora-ecommerce').'</h3></div><div class="related-posts-posts">';
										...
										<header class="entry-header">
											...
											if ( 'post' === get_post_type() ) :
												...
												<a class="read-more" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
													<?php echo esc_html_e('Read More','shopora-ecommerce'); ?>
												</a>
											<?php endif; ?>
										</div>
										<?php  ?>
