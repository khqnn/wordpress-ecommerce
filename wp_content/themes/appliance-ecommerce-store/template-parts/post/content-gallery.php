        ...
        <?php if (!is_single() && function_exists('get_post_gallery')) {
          ...
          if ($gallery && isset($gallery['ids'])) { 
            ...
            <div class="container entry-gallery">
              <div class="row">
                <?php $appliance_ecommerce_store_max_images = min(count($gallery_ids), 4);
                for ($index = 0; $index < $appliance_ecommerce_store_max_images; $index++) {
                  $id = $gallery_ids[$index];
                  $appliance_ecommerce_store_image_url = wp_get_attachment_image_url($id, 'full'); ?>
                  <div class="col-md-6 mb-1 align-self-center">
                    <img class="img-fluid" src="<?php echo esc_url($appliance_ecommerce_store_image_url); ?>" alt="Gallery Image <?php echo ($index + 1); ?>">
                  </div>
                <?php } ?>
              </div>
            </div>
