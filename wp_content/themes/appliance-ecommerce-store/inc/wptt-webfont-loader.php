...
if ( ! class_exists( 'Appliance_Ecommerce_Store_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Appliance_Ecommerce_Store_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.0.0
		 * @return array Returns an array of remote URLs and their local appliance_ecommerce_store_counterparts.
		 */
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $appliance_ecommerce_store_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $appliance_ecommerce_store_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$appliance_ecommerce_store_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$appliance_ecommerce_store_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$appliance_ecommerce_store_font_family = trim( str_replace( array( "'", ';' ), '', $appliance_ecommerce_store_font_family ) );
					$appliance_ecommerce_store_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $appliance_ecommerce_store_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $appliance_ecommerce_store_font_family ] ) ) {
					$result[ $appliance_ecommerce_store_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$appliance_ecommerce_store_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$appliance_ecommerce_store_font_family_url = $this->get_absolute_path( $appliance_ecommerce_store_font_family_url );

					$result[ $appliance_ecommerce_store_font_family ][] = $appliance_ecommerce_store_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $appliance_ecommerce_store_font_family ] = array_flip( array_flip( $result[ $appliance_ecommerce_store_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'appliance_ecommerce_store_wptt_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function appliance_ecommerce_store_wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Appliance_Ecommerce_Store_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'appliance_ecommerce_store_wptt_get_web_font_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function appliance_ecommerce_store_wptt_get_web_font_url( $url, $format = 'woff2' ) {
		$font = new Appliance_Ecommerce_Store_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
